<template>
    <div class="ds-table-wrapper">
        <!-- Header row: column count + loading indicator -->
                ...
                <tbody>
                    ...
                    <tr v-else-if="rows.length === 0">
                        <td :colspan="columns.length" class="ds-table__td ds-table__td--empty">
                            <slot name="empty">
                                <div class="ds-table-empty">No results found.</div>
                            </slot>
                        </td>
                    </tr>
                    ...
                    <!-- Data rows -->
                    <template v-else>
                        <tr v-for="(row, rowIndex) in rows" :key="rowIndex" class="ds-table__row">
                            <td v-for="col in columns" :key="col.key" class="ds-table__td">
                                <slot
                                    :name="`cell.${col.key}`"
                                    :row="row"
...
import { ref } from 'vue'
import type { TableColumn, PaginationMeta } from '@/ds/ui'
...
// Expose sort state so parent can read current sort for URL sync if needed
