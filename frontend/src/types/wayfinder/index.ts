export type QueryParams = {
  [key: string]: string | number | boolean | (string | number)[] | null | undefined | QueryParams
}
...
const addNestedParams = (obj: QueryParams, prefix: string, params: URLSearchParams) => {
...
export const applyUrlDefaults = <T extends UrlDefaults | undefined>(existing: T): T => {
  const existingParams = { ...(existing ?? ({} as UrlDefaults)) }
  const defaultParams = urlDefaults()

  for (const key in defaultParams) {
    if (existingParams[key] === undefined && defaultParams[key] !== undefined) {
      ;(existingParams as Record<string, unknown>)[key] = defaultParams[key]
    }
  }
